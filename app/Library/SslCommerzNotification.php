...
class SslCommerzNotification extends AbstractSslCommerz
{
    ...
    public function makePayment(array $requestData, $type = 'checkout', $pattern = 'json')
    {
        ...
        if ($type == 'hosted') {
            if (!empty($formattedResponse['GatewayPageURL'])) {
                ...
                if (isset($requestData['scan-checkout'])){
    ...
    {
        return rtrim(env('APP_URL').'/payment', '/') . $url;
    }
    protected function setSuccessUrl()
    {
        $this->successUrl = $this->setUrl($this->config['success_url']);
    }
    ...
    {
        $this->failedUrl = $this->setUrl($this->config['failed_url']);
    }
    ...
    {
        $this->cancelUrl = $this->setUrl($this->config['cancel_url']);
    }
    ...
    {
        $this->ipnUrl = $this->setUrl($this->config['ipn_url']);
    }
    ...
    public function setRequiredInfo(array $info)
    {
        ...
        $transactionId = $info['tran_id'];
        $this->data['success_url'] = $this->getSuccessUrl().'/'.$transactionId; // string (255)	Mandatory - It is the callback URL of your website where user will redirect after successful payment (Length: 255)
        $this->data['fail_url'] = $this->getFailedUrl().'/'.$transactionId; // string (255)	Mandatory - It is the callback URL of your website where user will redirect after any failure occure during payment (Length: 255)
        $this->data['cancel_url'] = $this->getCancelUrl().'/'.$transactionId; // string (255)	Mandatory - It is the callback URL of your website where user will redirect if user canceled the transaction (Length: 255)
		...
		*/
        $this->data['ipn_url'] = $this->getIPNUrl().'/'.$transactionId;
